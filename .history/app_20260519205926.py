...
def get_message(key, **kwargs):
    lang = 'en'
    if current_user and current_user.is_authenticated and hasattr(
...
def index():
    ...
    member_projects = [
        m.project for m in ProjectMember.query.filter_by(
...
def view_project(project_id):
    ...
    is_member = project.is_member(current_user)
    return render_template('project.html', project=project,
                           is_owner=is_owner, is_member=is_member)
...
def download_file(name):
    ...
    return send_from_directory(os.path.join(
        app.root_path, app.config['UPLOAD_FOLDER']), name)
...
def request_join(project_id):
    ...
    if existing_member:
        ...
        return redirect(url_for('view_project', project_id=project.id))
    pending = ProjectJoinRequest.query.filter_by(
...
def view_requests(project_id):
    ...
    pending_requests = ProjectJoinRequest.query.filter_by(
        project_id=project.id, status='pending').all()
    return render_template(
        'requests.html', project=project, requests=pending_requests)


@app.route('/project/<int:project_id>/requests/<int:request_id>/approve',
           methods=['POST'])
@login_required
...
@app.route('/project/<int:project_id>/requests/<int:request_id>/reject',
           methods=['POST'])
@login_required
...
def project_members(project_id):
    ...
    return render_template('members.html', project=project,
                           members=members, is_owner=is_owner)


@app.route('/project/<int:project_id>/remove_member/<int:member_id>',
           methods=['POST'])
@login_required
...
@login_required
def my_requests():
    requests = ProjectJoinRequest.query.filter_by(
        user_id=current_user.id).order_by(
        ProjectJoinRequest.created_at.desc()).all()
...
def get_avatar(name):
    return send_from_directory(os.path.join(
        app.root_path, app.config['UPLOAD_FOLDER'], 'avatars'), name)
